...
!> Performs the calculations for rainrate etc.
  subroutine dobulkmicrostat3
    ...
    do k = 1,k1
      ...
      raincountavl (k)  = count(q_hr    (2:i1,2:j1,k) > epsqr)
      preccountavl (k)  = count(precep_l(2:i1,2:j1,k) > epsprec)
      prec_prcavl  (k)  = sum  (precep_l(2:i1,2:j1,k), precep_l(2:i1,2:j1,k) > epsprec)
      Nrrainavl    (k)  = sum  (n_hr    (2:i1,2:j1,k)) * rhof(k)
      precavl      (k)  = sum  (precep_l(2:i1,2:j1,k))
      qravl        (k)  = sum  (q_hr    (2:i1,2:j1,k))
...
!> Performs the calculations for the tendencies etc.
  subroutine bulkmicrotend3
    ...
    use modmicrodata3, only : q_hrp, n_hrp                                  &
    ...
    real, dimension(:,:,:), allocatable  :: depos, subli,sadjpl,sadjneg, termtest
    integer        :: ifield = 0
  ...
  ! dnr_rime_srav = 0.0
    ...
    avfield    = 0.0
    call slabsum(avfield  ,1,k1,dn_hr_au  ,2-ih,i1+ih,2-jh,j1+jh,1,k1,2,i1,2,j1,1,k1)
    do k=1,k1
